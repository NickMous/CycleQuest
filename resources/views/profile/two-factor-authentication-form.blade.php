<x-action-section>
    ...
    <x-slot name="description">
        {{ __('profile.two_factor_authentication_description') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            @if ($this->enabled)
                ...
                @endif
            @else
                {{ __('profile.two_factor_authentication_disabled') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600 dark:text-gray-400">
            <p>
                {{ __('profile.two_factor_authentication_explanation') }}
            </p>
        </div>
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-gray-400">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __('profile.two_factor_authentication_instructions') }}
                        @else
                            {{ __('profile.two_factor_authentication_instructions_qr') }}
                        @endif
                    </p>
                </div>
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-gray-400">
                    <p class="font-semibold">
                        {{ __('profile.recovery_codes_description') }}
                    </p>
                </div>
