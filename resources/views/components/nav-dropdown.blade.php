@props(['align' => 'right', 'width' => '48', 'contentClasses' => 'py-1 bg-bg dark:bg-dm-bg', 'dropdownClasses' => '', 'active', 'outerDivClasses' => '', 'triggerDivClasses' => ''])
...
<div class="relative {{$outerDivClasses}}" x-data="{ open: false }" @click.away="open = false" @close.stop="open = false">
    <div @click="open = ! open" class="{{$triggerDivClasses}}">
        {{ $trigger }}
    </div>
    ...
    <div x-show="open"
            ...
            style="display: none;"
            @click="open = false">
        <div class="rounded-md ring-1 ring-black ring-opacity-5 dark:ring-dm-bg-600 {{ $contentClasses }}">
            {{ $content }}
        </div>
    </div>
</div>
